...
static __always_inline int bpf_create_map(enum bpf_map_type map_type,
...
static __always_inline int bpf_create_map_with_name(enum bpf_map_type map_type,
                                                    unsigned int key_size,
                                                    unsigned int value_size,
                                                    unsigned int max_entries,
                                                    const char *name) {
...
static __always_inline int bpf_lookup_elem(int fd, const void *key,
                                           void *value) {
  union bpf_attr attr = {
      .map_fd = fd,
      .key = (uint64_t)(key),
...
static __always_inline int bpf_update_elem(int fd, const void *key,
...
static __always_inline int bpf_delete_elem(int fd, const void *key) {
  union bpf_attr attr = {
      .map_fd = fd,
      .key = (uint64_t)(key),
  };
...
static __always_inline int bpf_obj_get_info_by_fd(int fd,
...
static __always_inline int bpf_prog_load(enum bpf_prog_type type,
...
bpf_prog_test_run(int prog_fd, int repeat, void *data, uint32_t size,
                  void *data_out, unsigned int *size_out, uint32_t *retval,
                  uint32_t *duration) {
  union bpf_attr attr;
  int ret;
...
static __always_inline int bpf_prog_skb_run(int prog_fd, const void *data,
                                            size_t size) {
  int err, socks[2] = {};
...
#define GET_MAP_ADDR(fd, reg)                                                  \
  BPF_MOV64_IMM(BPF_REG_0, 0), BPF_STX_MEM(BPF_W, BPF_REG_10, BPF_REG_0, -4),  \
      BPF_MOV64_REG(BPF_REG_2, BPF_REG_10),                                    \
      BPF_ALU64_IMM(BPF_ADD, BPF_REG_2, -4), BPF_LD_MAP_FD(BPF_REG_1, fd),     \
      BPF_RAW_INSN(BPF_JMP | BPF_CALL, 0, 0, 0, BPF_FUNC_map_lookup_elem),     \
      BPF_JMP_IMM(BPF_JNE, BPF_REG_0, 0, 1), BPF_EXIT_INSN(),                  \
      BPF_MOV64_REG((reg), BPF_REG_0), BPF_MOV64_IMM(BPF_REG_0, 0)
...
#define BPF_MAX_INSNS 0x500

int jmp_ops_codes[] = {
    BPF_JNE,  BPF_JLT, BPF_JLE, BPF_JSGT, BPF_JSGE, BPF_JSLT,
    BPF_JSLE, BPF_JA,  BPF_JEQ, BPF_JGT,  BPF_JGE,  BPF_JSET,
};
